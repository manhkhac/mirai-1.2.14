  ...
  switch (event.body) {
    case "1":
      api.sendMessage({
        ...
        attachment: fs.createReadStream(__dirname + `/Noprefix/12congiap/ti.gif`)
    ...
    case "6":
      api.sendMessage({
        ...
        attachment: fs.createReadStream(__dirname + `/Noprefix/12congiap/ran.gif`)
      }, event.threadID, event.messageID);
      break;
    case "7":
      api.sendMessage({
  ...
  arr.forEach(i => {
    ...
    if (body === i.toUpperCase() | body === i | str === body) {
      api.sendMessage({
        ...
        attachment: fs.createReadStream(__dirname + `/Noprefix/12congiap/12congiap.jpg`)
