...
•Ví dụ: Noprefix a
•Key: để trống
•Tác dụng: lọc ra tất cả các file trong Noprefix
•Ví dụ: Noprefix
•Key: help
...
•Ví dụ: Noprefix help`;
    ...
    if(files.length == 0) return api.sendMessage(`Không có file nào trong Noprefix có ký tự bắt đầu bằng: ${word}`, event.threadID ,event. messageID);
  ...
  //đuôi file là..... 
  	...
  	if(files.length == 0) return api.sendMessage(`Không có file nào trong Noprefix có ký tự kết thúc bằng: ${ext}`, event.threadID ,event. messageID);
  	var key = `Có ${files.length} file có đuôi là: ${ext}`;
  }
  //all file
  else if (!args[0]) {
  if(files.length == 0) return api.sendMessage("Noprefix của bạn không có file hoặc folder nào", event.threadID ,event. messageID);
