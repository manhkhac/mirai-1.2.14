module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "NTKhang",
    description: "tự động cấm người dùng nếu spam bot 6 lần/180s",
...
module.exports.run = ({ api, event, args, Users, Threads }) => {
    return api.sendMessage("Tự động cấm người dùng nếu spam bot 6 lần/3 phút", event.threadID, event.messageID);
};
...
module.exports.handleEvent = async function({ api, event, args, Users, Threads }) {
    ...
    const prefixDefaut = threadSetting.PREFIX || global.config.PREFIX;
    //////////////////////////  manhG start
    //prefix == null ? prefix = `${prefixDefaut}` : prefix = `${data.PREFIX}`;
    ...
    if ((global.client.autoban[senderID].timeStart + 180000) <= Date.now()) {
        ...
        if (global.client.autoban[senderID].number >= 6) {
            ...
            var reason = "spam bot 6 lần/3 phút";
            data.banned = true;
